...
import { emailVerificationContent, forgotPasswordContent, sendEmail, } from "../services/sendMail.services.js";
...
export const register = asyncHandler(async (req, res) => {
    ...
    console.log(isOrg);
    if (!isOrg)
        throw new ApiError(400, "Org is not found!");
    ...
    const data = await UserModel.findById(newUser._id).select("-password -verificationToken -resetPasswordToken -refreshToken");
    if (!data)
        throw new ApiError(500, "Something went wrong while Register a User.");
    return res
        .status(200)
        .json(new ApiResponse(200, { user: data }, "User added successfully and verification email send"));
});
export const login = asyncHandler(async (req, res) => {
    ...
    const loggedUser = await UserModel.findById(user._id).select("-password -verificationToken -resetPasswordToken -refreshToken");
    const options = {
        httpOnly: true,
        secure: true,
    };
...
export const resendEmailVerification = asyncHandler(async (req, res) => {
    ...
    if (user.isVerified)
        throw new ApiError(400, "User alredy verified.");
    ...
    await sendEmail({
        ...
        mailgenContent: emailVerificationContent(user?.name, `${req.protocol}://${req.get("host")}/api/v1/user/verify-email/${unHashedToken}`)
    });
...
export const forgotPassword = asyncHandler(async (req, res) => {
    ...
    await sendEmail({
        ...
        mailgenContent: forgotPasswordContent(user.name, `${process.env.FORGOT_PASSWORD_REDIRECT_URL}/${unHashedToken}`)
    });
    return res.status(200).json(new ApiResponse(200, {}, "Password reset mail has been sent on your mail."));
});
