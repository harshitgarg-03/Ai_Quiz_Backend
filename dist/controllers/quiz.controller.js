...
export const createQuiz = asyncHandler(async (req, res) => {
    ...
    const user = req.user;
    if (!topic || !quizType)
        throw new ApiError(400, "Topic and type are required!");
    ...
    if (!quizData)
        throw new ApiError(400, "Quiz generation failed!");
...
export const updateQuiz = asyncHandler(async (req, res) => {
    const orgId = req.user.organizationId;
    const quiz = await QuizModel.findOneAndUpdate({ _id: req.params.id, organizationId: orgId }, { $set: req.body }, { new: true });
    if (!quiz)
        throw new ApiError(404, "Quiz not found!");
